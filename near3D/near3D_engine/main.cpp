...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_  HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd) {
	...
	DebugClass::Create(Frame_Rate);					//デバッグ
	...
	do {
		...
		while (ProcessMessage() == 0) {
			...
			DebugClass::Instance()->put_way();
			//入力
			{
				SetJoypadDeadZone(DX_INPUT_PAD1, 0.0);
				GetJoypadDirectInputState(DX_INPUT_PAD1, &info);
				KEY[(int)Key::KEY_PAUSE].GetInput(CheckHitKeyWithCheck(KEY_INPUT_P) != 0);
				...
				if (KEY[(int)Key::ON_PAD].press()) {
					...
					KEY[(int)Key::KEY_M_LEFT].GetInput((GetMouseInputWithCheck() & MOUSE_INPUT_LEFT) != 0);
					KEY[(int)Key::KEY_M_MID].GetInput((GetMouseInputWithCheck() & MOUSE_INPUT_MIDDLE) != 0);
					KEY[(int)Key::KEY_M_RIGHT].GetInput((GetMouseInputWithCheck() & MOUSE_INPUT_RIGHT) != 0);
					KEY[(int)Key::KEY_NO_1].GetInput(CheckHitKeyWithCheck(KEY_INPUT_LSHIFT) != 0);
					KEY[(int)Key::KEY_NO_2].GetInput(CheckHitKeyWithCheck(KEY_INPUT_SPACE) != 0);
					KEY[(int)Key::KEY_NO_3].GetInput(CheckHitKeyWithCheck(KEY_INPUT_R) != 0);
					KEY[(int)Key::KEY_NO_4].GetInput(CheckHitKeyWithCheck(KEY_INPUT_X) != 0);
					KEY[(int)Key::KEY_NO_5].GetInput(CheckHitKeyWithCheck(KEY_INPUT_L) != 0);
					KEY[(int)Key::KEY_NO_6].GetInput(CheckHitKeyWithCheck(KEY_INPUT_F) != 0);
					KEY[(int)Key::KEY_UP].GetInput((CheckHitKeyWithCheck(KEY_INPUT_W) != 0 || CheckHitKeyWithCheck(KEY_INPUT_UP) != 0));
					KEY[(int)Key::KEY_DOWN].GetInput((CheckHitKeyWithCheck(KEY_INPUT_S) != 0 || CheckHitKeyWithCheck(KEY_INPUT_DOWN) != 0));
					KEY[(int)Key::KEY_LEFT].GetInput((CheckHitKeyWithCheck(KEY_INPUT_A) != 0 || CheckHitKeyWithCheck(KEY_INPUT_LEFT) != 0));
					KEY[(int)Key::KEY_RIGHT].GetInput((CheckHitKeyWithCheck(KEY_INPUT_D) != 0 || CheckHitKeyWithCheck(KEY_INPUT_RIGHT) != 0));
				}
			}
			//出力
			{
				...
				if (KEY[(int)Key::KEY_M_MID].press()) {
					...
					if (RM_PressTimer > 0.1f) {
						...
						Easing(&RM_PX, (float)-x_m / 2, 0.9f, EasingType::OutExpo);
						Easing(&RM_PY, (float)-y_m / 2, 0.9f, EasingType::OutExpo);
					}
				}
				else {
					RM_PressTimer = 0.f;
					Easing(&RM_PX, 0.f, 0.9f, EasingType::OutExpo);
					Easing(&RM_PY, 0.f, 0.9f, EasingType::OutExpo);
				}
				Easing(&X, (float)(Near3DPts->PlayerPos()*-1.f).x + RM_PX, 0.925f, EasingType::OutExpo);
				Easing(&Y, (float)(Near3DPts->PlayerPos()*-1.f).y + RM_PY, 0.925f, EasingType::OutExpo);
			...
			GraphHandle::SetDraw_Screen((int)DX_SCREEN_BACK);
			{
				...
				Near3DPts->Output_UI();
				//
				DebugClass::Instance()->end_way();
				DebugClass::Instance()->debug(10, 100, float(GetNowHiPerformanceCount() - waits) / 1000.f);
			}
			//画面の反映
